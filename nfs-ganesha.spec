...
name-spaces. This version is compiled to support the fuse back-end.
...
%package fsal-fuse
Summary: The NFS-GANESHA's FSAL dedicated to fuse, provided as a shared object
Group: Applications/System
Requires: %{name}-common = %{version}-%{release}

%description fsal-fuse
...
%configure  '--with-fsal=FUSE' '--enable-nlm' 'CC=gcc -g' "CFLAGS=$CFLAGS -g" 'CXXFLAGS=-g'
...
%if %{?_with_fuse:0}%{!?_with_fuse:1}
install -m 755 nfs-ganesha.fuse.init                      $RPM_BUILD_ROOT%{_sysconfdir}/init.d/nfs-ganesha-fuse
%endif
...
%if %{?_with_sharedfsalyes:1}%{!?_with_sharedfsalyes:0}
%if %{?_with_fuse:0}%{!?_with_fuse:1}
rm -f $RPM_BUILD_ROOT%{_libdir}/libfsalfuse.la
rm -f $RPM_BUILD_ROOT%{_libdir}/libfsalfuse.a
%endif
%endif
